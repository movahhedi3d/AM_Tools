...
def Am_ikfk_switch_func( *args):
    key = args[0]
    relative = args[1]
    ikfk_switch.switch_func(key, relative)
...
class Am_viewportMenu(object):
    ...
    def buildMenuItems(self):
        ...
        mc.menuItem(p= xRay_subMenu ,rp='W', l='X_Ray' , ecr=True , c =partial( self.__class__.toggle_mode , self.panel , "xray") , i = 'Am_XRay_32.png')
        mc.menuItem(p= xRay_subMenu ,rp='S', l='joint X_ray' , ecr=True , c = partial( self.__class__.toggle_mode , self.panel , "joint") , i = 'Am_XRayJoint_32.png')
        ...
        if is_AmRig and userAttr and 'match_nodes' in userAttr :
            ...
            mc.menuItem(p= self.menu , l='Switch Ik/Fk' , ecr=True , c = partial( Am_ikfk_switch_func , False , False) , i = "Am_IkFkSwitch_32.png")
            mc.menuItem(p= self.menu , l='Switch Ik/Fk + Key' , ecr=True , c = partial( Am_ikfk_switch_func , True , False) , i = "Am_IkFkSwitch_key_32.png")
                
        if is_AmRig and userAttr and 'match_nodes' in userAttr and 'has_relatives' in userAttr:
            mc.menuItem(p= self.menu , divider=True)
            mc.menuItem(p= self.menu , l='Switch Ik/Fk [match relatives]' , ecr=True , c = partial( Am_ikfk_switch_func , False , True) , i = "Am_IkFkSwitch_32.png")
            mc.menuItem(p= self.menu , l='Switch Ik/Fk [match relatives] + Key ' , ecr=True , c = partial( Am_ikfk_switch_func , True , True) , i = "Am_IkFkSwitch_key_32.png")
    ...
    def get_CompSetting(*args):
        ...
        if sel and mc.attributeQuery( 'AmCtrl', n=sel[0] , ex=True ):
            item = sel[0]
            setting_node = None
            while True:
                parent = mc.listRelatives(item, p=True)
                if parent and mc.attributeQuery( 'componentType', n= parent[0] , ex=True ):
                    setting_node = parent[0]
                    break
                elif parent:
                    item = parent[0]
                else:
    ...
    @classmethod
    def toggle_mode(cls , *args):
        
        panel = args[0]
        mode = args[1]
        
        if mode == 'xray':
            newState = not mc.modelEditor( panel, query=True, xray=True )
            mc.modelEditor( panel, edit=True, xray= newState)
        
        elif mode == 'wire':
            newState = not mc.modelEditor( panel, query=True, wos=True )
            mc.modelEditor( panel, edit=True, wos= newState)

        elif mode == 'joint':
            newState = not mc.modelEditor( panel, query=True, jx=True )
            mc.modelEditor( panel , edit=True, jx= newState)
