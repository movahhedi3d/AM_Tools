...
from AM_MENU.extensions import attribute
from AM_MENU.extensions import callbackManager

from AM_MENU.extensions.Qt import QtGui
from AM_MENU.extensions.Qt import QtCore
from AM_MENU.extensions.Qt import QtOpenGL
from AM_MENU.extensions.Qt import QtCompat
from AM_MENU.extensions.Qt import QtWidgets

from AM_MENU.extensions.six.moves import range
...
class APPassthroughEventFilter(QtCore.QObject):
    ...
    def eventFilter(self, QObject, event):
        ...
        if QtCompat.isValid(self.APUI):
            ...
            elif flag_state and not get_option_var_passthrough_state():
                self.APUI.set_mouseEvent_passthrough(False)
            else:
                pass
        else:
...
class GraphicViewWidget(QtWidgets.QGraphicsView):
    ...
    def convert_picker_to_curves(self):
        ...
        if cmds.objExists(PICKER_EXTRACTION_NAME):
            grp = PICKER_EXTRACTION_NAME
        else:
            grp = cmds.group(em=True, n=PICKER_EXTRACTION_NAME)
            attribute.lockAttribute(grp)

        # deletes existing tab group and recreates it
        if cmds.objExists(tab["name"]):
            cmds.delete(tab["name"])
        picker_grp = cmds.group(em=True, n=tab["name"], p=grp)
        cmds.connectAttr(picker_grp+".sy", picker_grp+".sx")
        attribute.lockAttribute(picker_grp, ["tz", "rx", "ry", "rz", "sx", "sz", "v"])
        ...
        if "background" in tab["data"]:
            ...
            ip = cmds.imagePlane(n="{}_background".format(tab["name"]))
            cmds.setAttr(ip[0]+".tz" , -1)
            cmds.setAttr(ip[0]+".overrideEnabled" , 1)
            cmds.setAttr(ip[0]+".overrideDisplayType" , 2)
            cmds.parent(ip[0], picker_grp)
            
            cmds.connectAttr( picker_grp+".backgroundAlpha", ip[1]+".alphaGain" )
            cmds.connectAttr( picker_grp+".backgroundWidth", ip[1]+".width" )
            cmds.connectAttr( picker_grp+".backgroundHeight", ip[1]+".height" )
            
            cmds.setAttr(ip[1]+".imageName", tab["data"]["background"])
        ...
        if "items" in tab["data"]:
            for item in tab["data"]["items"]:
                ...
                if len(handles) > 2:
                    item_curve = cmds.circle(d=1, s=len(
                        item["handles"]), ch=False)[0]

                    curve_shp = cmds.listRelatives(item_curve , s=True)[0]
                    for i, (x, y) in enumerate(handles):
                        cmds.setAttr(curve_shp+".controlPoints[{}]".format(i) , x, y, 0)
                    
                    cmds.setAttr(curve_shp+".controlPoints[{}]".format(i+1) , handles[0][0], handles[0][1], 0)
                    

                # special case for circles
                elif len(handles) == 2:
                    item_curve = cmds.curve(p=[[handles[0][0],
                                              handles[0][1],
                                              0.0],
                                             [handles[1][0],
                                              handles[1][1],
                    ...
                    cmds.connectAttr(curve_shp+".worldSpace" , poci+".inputCurve")
                    
                    # item_curve.getShape().worldSpace >> poci.inputCurve
                    curve_len = cmds.arclen(item_curve, ch=True)
                    
                    display_curve = cmds.circle(d=3, s=6, ch=False)[0]
                    cmds.parent(display_curve, item_curve)
                    
                    display_curve_shp = cmds.listRelatives(display_curve , s=True)[0]
                    cmds.setAttr(display_curve_shp+".overrideEnabled", 1 )
                    cmds.setAttr(display_curve_shp+".overrideDisplayType", 2 )
                    cmds.setAttr(display_curve+".inheritsTransform", 0 )
                    
                    cmds.connectAttr(curve_len+".arcLength", display_curve+".sx" )
                    cmds.connectAttr(curve_len+".arcLength", display_curve+".sy" )
                    cmds.connectAttr(curve_len+".arcLength", display_curve+".sz" )
                    cmds.connectAttr(poci+".position", display_curve+".t" )
                    
                cmds.parent(item_curve, picker_grp)
                ...
                cmds.setAttr(item_curve+".t", pos_x, pos_y, 0 )
                cmds.setAttr(item_curve+".rz", rot_z)
                cmds.setAttr(item_curve+".displayHandle", 1 )
                
                curve_shp = cmds.listRelatives(item_curve , s=True)[0]
                cmds.setAttr(curve_shp+".dispCV", 1 )
                cmds.setAttr(item_curve+".overrideEnabled", 1 )
                cmds.setAttr(item_curve+".overrideRGBColors", 1 )
                cmds.connectAttr(item_curve+".color", item_curve+".overrideColorRGB" )
                cmds.connectAttr(item_curve+".scalePivot", item_curve+".selectHandle" )
                ...
                if item_data:
                    ...
                    cmds.setAttr(item_curve+".itemData", lock=True )

    def delete_extraction_grp(self):
        """delete extraction group
        """
        try:
            cmds.delete(PICKER_EXTRACTION_NAME)
    ...
    def convert_curves_to_picker(self):
        ...
        for tab_grp in cmds.listRelatives(grp):
            new_data["tabs"].append({"name": tab_grp})
            new_data["tabs"][-1]["data"] = {"items": []}
            bg_imagePlane = cmds.listRelatives(tab_grp , type="imagePlane", ad=True )

            if bg_imagePlane:
                image_name = cmds.getAttr(bg_imagePlane[0]+".imageName" )
                new_data["tabs"][-1]["data"]["background"] = image_name
                new_data["tabs"][-1]["data"]["background_size"] = [
                    cmds.getAttr(bg_imagePlane[0]+".width"),
                    cmds.getAttr(bg_imagePlane[0]+".height")]
            
            
            
            for item_curve in [ic for ic in cmds.listRelatives(tab_grp) if 
                        cmds.objectType(cmds.listRelatives(ic, s=True)[0]) != "imagePlane"]:
                item_data = {}
                
                # color
                q_color = QtGui.QColor()
                q_color.setRgbF(*cmds.getAttr(item_curve+".color")[0])
                q_color.setAlpha(cmds.getAttr(item_curve+".alpha"))
                item_data["color"] = q_color.getRgb()
                
                # position and rotation
                item_piv = cmds.xform(item_curve, q=True, pivots=True, ws=True)
                item_m = cmds.xform(item_curve, q=True, m=True, ws=True)
                tab_grp_m = cmds.xform(tab_grp, q=True, m=True, ws=True)
                
                o_item_piv = om.MVector(item_piv[:3])
                o_item_m = om.MMatrix(item_m)
                o_tab_grp_m = om.MMatrix(tab_grp_m)
                
                piv_offset = o_item_piv * o_item_m.inverse()
                item_pos = o_item_piv * o_tab_grp_m
                

                item_data["position"] = [item_pos.x, item_pos.y]
                item_data["rotation"] = cmds.getAttr(item_curve+".rz")
                
                # handles
                handles = []
                
                
                item_scale = [cmds.getAttr(item_curve+".sx" ), cmds.getAttr(item_curve+".sy" )]
                curve_shp = cmds.listRelatives(item_curve , s=True)[0]
                
                curveCVs = cmds.ls('{0}.cv[:]'.format(curve_shp), fl = True)
                cv_pos = [cmds.pointPosition(x , l= True) for x in curveCVs]
                
                for cv in curveCVs:
                    x , y = cmds.pointPosition(cv , l= True)[:2]
                    handles.append(
                ...
                if cmds.attributeQuery("itemData", n=item_curve , ex=True ):
                    item_data.update(json.loads(cmds.getAttr(item_curve+".itemData" )))
        ...
        cmds.setAttr( current_node+".picker_datas", lock=False )
        cmds.setAttr( current_node+".picker_datas", lock=False )
        cmds.setAttr( current_node+".picker_datas", json.dumps(new_data).replace("true", "True"), type='string')
        cmds.setAttr( current_node+".picker_datas", lock=True )
...
class MainDockWindow(QtWidgets.QWidget):
    ...
    def toggle_passthrough_eventFilter(self):
        ...
        if get_option_var_passthrough_state() == 0 or not self.window_parent:
            return
            ...
            if event.type() == QtCore.QEvent.Type.Leave:
                ...
                if opacity_state:
                    ...
                    if not widgetRect.contains(pos):
                        ...
                        if get_option_var_passthrough_state():
