...
def AM_toolsMenuLoader():
    ...
    AM_custom_tools_menu = mc.menu(menu_obj , label=menu_label , parent=main_window, tearOff=True )
    
    mc.menuItem(label = 'Set Hotkey' , command = partial( setHotkey ) , i = "Am_menu_Hotkey_32.png")
    mc.menuItem(label = 'Anim Picker' , subMenu=True , parent=AM_custom_tools_menu , tearOff=True , i = "Am_select_32.png")
    mc.menuItem(label = 'Anim Picker' , command = partial(anim_picker.load , False) )
    mc.menuItem(label = 'Edit Anim Picker' , command = partial(anim_picker.load , True))
    mc.menuItem( label = 'Help' , divider=True)
    mc.menuItem(label = 'About Anim Picker' , command = partial( aboutAnimPicker ))
    mc.setParent('..' , menu=True)
        
    mc.menuItem( label = 'Help' , divider=True)
    mc.menuItem(label = 'Contact Us' , command = partial(webbrowser.open , 'https://movahhedi.com/contact/') , i = "Am_contact1_32.png")
    mc.menuItem(label = 'Check for Updates' , command = partial( AM_MENU.checkForUpdate ))
    mc.menuItem(label = 'About' , command = partial( AM_MENU.about_AM_Tools ))
...
def about_AM_Tools(*args):
    ...
    AM_Tools is a lightweight and free utility developed under the MIT License. 
    It was created by Seyed Abolfazl Movahhedi as part of the AM_Rig Tool suite 
    to enhance character rigging workflows.

    Features:
        Compatible with all characters and rigs developed using AM_Rig Tool.
        Limited features are available for rigs outside the AM_Rig framework.

    Copyright (c) 2021 Seyed Abolfazl Movahhedi
    For detailed information and tutorials, visit the following link:
    https://movahhedi.com

    """.format(AM_Tools_ver)
    mc.confirmDialog(title='About AM_Tools', message=note, button=["OK"],
